...
% (Clsnum = 2 or 4)
% 21 Warped Photo Test; 22 Cut Photo Test; 23 Video Test
% (Clsnum = 2)
% 3  Overall Test
% (Clsnum = 2 or 4)
...
%% Testing using SVM
% for k = 3:3
%     [prob,Test,ACC_avg] = Fun_Classification_SVM_CASIA(TestType,Clsnum,T,k,Ptrain);
%     PROB = [PROB prob];
%     TEST = [TEST Test];
%     ACC = [ACC ACC_avg];
% end
...
%% Testing using SAEs
for k = 3:3
    [prob,Test,ACC_avg] = Fun_Classification_SAE_CASIA(TestType,Clsnum,T,k,Ptrain);
    PROB = [PROB prob];
    TEST = [TEST Test];
    ACC = [ACC ACC_avg];
end
