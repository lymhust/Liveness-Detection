function [proball,TestLabelAll,ACC_avg] = Fun_Classification_SVM_CASIA(TestType,Clsnum,T,Method,Ptrain)
...
switch TestType
    case 11
        ...
        switch Method
            ...
            case 4
                if Clsnum == 2
                    load LOW_ESBFD_S3_D27_B64_Cls2;
                    gnum = 6;
                else
        ...
        switch Method
            ...
            case 3
                if Clsnum == 2
                    ...
                    %load HR_FaceFeaturesC;%HR_FaceFeaturesNew_256by256_64_mean_2class;
                    gnum = 60;
                else
...
for k = 1:T
    ...
    %% Testing
    svmStruct = svmtrain(TrainLabels,TrainImg');%svmStruct = svmtrain(TrainImg',TrainLabels);
    [pred, accuracy, prob] = svmpredict(TestLabels, TestImg', svmStruct);%pred = svmclassify(svmStruct,TestImg');
    ...
    if (Method == 4 && Clsnum == 4) || (Method == 4 && TestType == 21) || (Method == 4 && TestType == 22) || (Method == 4 && TestType == 23)
        ...
        pred = round(mean(reshape(pred,[rsizenum,length(pred)/rsizenum])));
        TestLabels = mean(reshape(TestLabels,[rsizenum,length(TestLabels)/rsizenum]));
        prob = mean(reshape(prob,[rsizenum,length(prob)/rsizenum]));
        acc = mean(TestLabels' == pred')
        ACC_avg = [ACC_avg;acc];
        proball = [proball;prob'];
        TestLabelAll = [TestLabelAll;TestLabels'];
